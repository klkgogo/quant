...
ENV_REAL = ft.TrdEnv.REAL
ENV_SIMULATE = ft.TrdEnv.SIMULATE


class FutuQuantFrame(object):
    """策略frame"""
    # settingFileName = 'setting.json'
    # settingfilePath = getJsonPath(settingFileName, __file__)

    def __init__(self, api_ip, api_port, market, env_type=ENV_SIMULATE, trade_passwod=""):
        """frame settings"""
        self._api_ip = api_ip
        self._api_port = api_port
        self._market = market
        self._env_type = env_type
        self._trade_password = trade_passwod
    ...
    #
    def get_snapshot(self, symbol_list):
        ret, data = self._quote_ctx.get_market_snapshot(symbol_list)
        if ret != ft.RET_OK:
            return ret, data
        return data
    ...
    def on_quote_change(self, data):
        self._tiny_strate.on_quote_changed(data)

    def init(self, strate):
        self._tiny_strate = strate
        self._process_init_api()

    def close(self):
        self._quote_ctx.close()
        self._trade_ctx.close()
        self._tiny_strate.on_strate_close()
    ...
    def _process_init_api(self):
        ...
        #定阅行情数据
        self._futu_data_event = FutuDataEvent(self, self._quote_ctx, self._tiny_strate.symbol_pools, self._tiny_strate.subtype_list)
